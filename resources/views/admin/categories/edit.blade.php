                            ...
                            <form action="{{ route('categories.update', $category->id) }}" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_ru" class="form-control "
                                        value="{{ old('name', $category->name_ru) }}" required="">
                                    <span class="error invalid-feedback"></span>
                                </div>
                                <div class="form-group">
                                    <label>Название на узбекском</label>
                                    <input type="text" name="name_uz" class="form-control"
                                        value="{{ old('name', $category->name_uz) }}" required="">
                                </div>
                                <div class="form-group">
                                    <label>Главние Категория</label>
                                    <select class="form-control" name="head_category_id">
                                        @foreach ($headcategories as $headcategory)
                                            <option {{ $category->head_category_id == $headcategory->id ? 'selected' : '' }}
                                                value="{{ $headcategory->id }}">{{ $headcategory->name_ru }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
